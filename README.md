...
文件夹模式专用于剧集：它会先检查全部目标文件名，支持 `S01E01`、`S01E01-E02` 和 `S01E01E02`（自动规范为 `S01E01-E02`）等季集写法；任何一集缺少季集号、根目录缺少年份或发生重名时都不会改动任何文件。执行 `--apply` 时会按 M-Team 的剧集文件夹规则，将根目录改为 `{title}-{year}-{season}-[tmdb={tmdb_id}]`（没有可用 TMDB ID 时省略最后一段），例如 `House of Cards-2013-S06-[tmdb=1425]`。如果输入父目录本身已经明确写有单季（如 `Show S01`、`Show Season 1` 或 `第1季`），文件会直接放在改名后的单季根目录中，不再重复建立 `Season 01`；只有多季打包（如 `Show S01-S05` 或同时包含多个季）才自动建立 `Season 01`、`Season 02` 等分季子文件夹。季数属于根目录身份，因此同一剧集的不同季不会互相误判为已存在目标，也不会在目录种子阶段共享错误的 torrent 根名。文件按季集号排序后只探测第一集并生成一份 MediaInfo Text，其分辨率、视频编码和音频参数会用于整季重命名及发布页；若代表集文件名没有年份，会从全部集的四位年份标记中选取最常见值，避免 TMDB 暂时不可用时无法生成目录名。若长任务已有续传检查点，即使本次 TMDB 网络请求失败，也会从同一组源文件的检查点恢复原 TMDB ID，避免因目录逻辑名变化而拒绝续传。TMDB 主条目只查询一次，并在需要时再查询一次目标季信息，豆瓣会按目标季名称和季数匹配。默认的 4 张截图仍会尽量均匀选自不同集。最终生成一个 V1 私有多文件种子，种子根目录和目录结构都与改名后的结果一致。
